...
<p class="tagline">~900ms search | Vector + BM25 | Multimodal | OCR | Reranked results | Python Workers Runtime</p>
...
<button onclick="searchImages()">Search: "dashboard navigation"</button>
...
async function compressImage(file, maxWidth, maxHeight, quality) {
    maxWidth = maxWidth || 1920; maxHeight = maxHeight || 1080; quality = quality || 0.85;
...
async function searchFinancial(){ document.getElementById('searchQuery').value = 'Access Bank N30000'; await search(); var dl = document.getElementById('demoLog'); dl.style.display = 'block'; dl.innerHTML = '<span class="success">Search complete!</span> OCR extracted transaction details from receipt images.'; }

document.getElementById('searchQuery').addEventListener('keypress', function(e) { if(e.key === 'Enter') search(); });
loadStats();
testAuth();
</script>
</body>
